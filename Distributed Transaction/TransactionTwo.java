...
public class TransactionTwo extends Thread{
	...
	public TransactionTwo(Connection localConnection,Connection remoteConnection,String cityName,double freightValue,int heigth,LockTable lock) {
	...
	public void run() {
		System.out.println(Thread.currentThread().getName()+" Started Performing Transaction");
		selectSellerData();
		selectCusomerData();
		...
		updateSellerTable();
		System.out.println(Thread.currentThread().getName()+" Performance Done Operation");
		commitTransaction();
	}

	//Fetching the Customer Information
	private void selectCusomerData() {
		...
		try {
			System.out.println(Thread.currentThread().getName()+" performing selection on Customer Table");
	...
	//Fetching the Seller Information
	private void selectSellerData() {
		List<HashMap<String, Object>> listOne = new ArrayList<HashMap<String, Object>>();
		ConvertResultSetToHashMap rsToList = new ConvertResultSetToHashMap();
		try {
			System.out.println(Thread.currentThread().getName()+" performing selection on Seller Table");
	...
	//Update the Height of the product for a category
	private void updateProductDetails() {
		synchronized(this) {
			System.out.println("Updating PRODUCTS TABLE---------------------------");
			System.out.println(Thread.currentThread().getName()+" performing operation on product table");
			while(lock.getProductsTable()) {
				System.out.println(Thread.currentThread().getName()+" waiting for resource product table");
				try {
					Thread.sleep(500);
			...
			lock.setProductsTable(true);
			SqlFile sql = new SqlFile();
			List<HashMap<String, Object>> listOne = new ArrayList<HashMap<String, Object>>();
			ConvertResultSetToHashMap rsToList = new ConvertResultSetToHashMap();
			//Fetching the product_id from table
			try {
				System.out.println(Thread.currentThread().getName()+" Selecting product from the table");
				PreparedStatement ps = remote.prepareStatement(sql.loadProductDetails);
				ResultSet res = ps.executeQuery();
				listOne = rsToList.resultSetToArrayList(res);
			} catch (SQLException e1) {
				closeConnection();
			...
			//Updating Height for those data
			for(HashMap<String, Object> record : listOne) {
				String productId = record.get("product_id").toString();
				try {
					PreparedStatement update = remote.prepareStatement(sql.updateProdcutDetails);
					update.setInt(1, height);
					update.setString(2,productId);
					System.out.println(Thread.currentThread().getName()+" updating the Product Table");
					int value = update.executeUpdate();
					System.out.println(value+" record updated");
				} catch (SQLException e) {
					System.out.println(" Product Update Operation is failed");
			...
			System.out.println(Thread.currentThread().getName()+"Customer Table Update Done");
	...
	private void closeConnection() {
		try {
			System.out.println(Thread.currentThread().getName()+"Suspensed due to failure T2");
			local.rollback();
			remote.rollback();
